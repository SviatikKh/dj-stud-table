...
class UserManager(BaseUserManager):
    ...
    def create_user(self, email, password, *args, **kwargs):
        if not email:
            raise ValueError("Необхідна електронна адреса")
    ...
    def create_superuser(self, email, password, *args, **kwargs):
        user = self.model.create(email=email, password=password, *args, **kwargs)
        user.role = 1
        user.is_active = 1
...
class CustomUser(AbstractBaseUser):
    """
           ...
           param role: user role, default role (0, 'secretary')
    ...
    def get_by_id(user_id):
        ...
        try:
            ...
            return f'There is not user with ID{user_id}'
    ...
    def get_absolute_url(self):
        return reverse('get_user', kwargs={'id': self.id})
